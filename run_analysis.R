...
# only in my machine  # setwd("C:/Laurins/Safari_Downloads/00. AI-BI-DataManing-MachineLearning/__3.Getting.and.Cleaning.Data/3.Project/wk3/UCI HAR Dataset")
...
# features
    features <- read.table(file="features.txt", as.is=TRUE)

# Train
    subject_train <- read.table(file="train/subject_train.txt")
    
    raw_y_train       <- read.table(file="train/y_train.txt")
...
# Test
    ...
    raw_y_test   <- read.table(file="test/y_test.txt")
...
#####################################################################
    ...
    features_all <- rbind(c(1, "flagTrainTest"), c(2, "Subject"), c(3, "Activity"), features_all)   
...
# Train + Test
    dataFull <- rbind (Train_full, Test_full)     
            remove(Train_full,Test_full,flagTrainTest,x_test,x_train,y_test,y_train,subject_train,subject_test)
...
##########################################################################################
    ...
    df_Only_mean_std <- subset(x=tmp_Only_mean_std, select=features_OK[, 2] )  # keep only "good" vars
...
#######################################################################
    ...
    write.table(x=df_Mean_flagTrainTest_Subject_Activity, file="df_Mean_flagTrainTest_Subject_Activity.txt")
